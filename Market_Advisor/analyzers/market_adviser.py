...
class MarketAdviser:
    def __init__(self, model_dir='analyzers/models'):
        ...
        self.features = [
            ...
            'RollingMean_Close_3', 'RollingStd_Close_3',
            'RollingMean_High_3', 'RollingMean_Low_3',
    ...
    def preprocess_data(self, data):
        ...
        data['RollingMean_Close_3'] = data['Close'].rolling(window=3).mean()
        data['RollingStd_Close_3'] = data['Close'].rolling(window=3).std()
        data['RollingMean_High_3'] = data['High'].rolling(window=3).mean()
        data['RollingMean_Low_3'] = data['Low'].rolling(window=3).mean()
    ...
    def train_model(self, dataset_path, epochs=100, batch_size=32, transformer=False, window_size=10):
        """Train the model using the dataset and save it."""
        data = pd.read_csv(dataset_path)
        data = self.preprocess_data(data)
        data = self.calculate_profit_label(data)
        X, y = [], []
        ...
        print_colored_sentence(f"Model trained and saved at {self.model_path}")
        final_acc = self.model.evaluate(X_test, y_test, verbose=0)[1]
        print_colored_sentence(f"Final Test Accuracy: {final_acc:.2f}")
    ...
    def load_model(self):
        ...
        if os.path.exists(self.model_path):
            ...
            print_colored_sentence("Model loaded.")
        else:
            raise FileNotFoundError("Model not found. Train the model first.")

    def predict_last_recommended_action(self, dataset_path, window_size=10):
