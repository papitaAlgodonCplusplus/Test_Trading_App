...
def update_plot(t, price, action, x_vals, y_vals, hold_signals, buy_signals, sell_signals,
                current_portfolio_value, initial_portfolio_value, dates, total_gains, total_losses,
                place_buy_signals, place_sell_signals, pending_orders_signals, sell_instant_signals,
                sell_when_feasible_1_signals, sell_when_feasible_2_signals, reward):
    """
    Updates the plot data based on the current action and portfolio state.

    Parameters:
        t (int): Current time step.
        price (float): Current stock price.
        action (int): Action taken.
        x_vals (list): List of x-axis values (dates).
        y_vals (list): List of y-axis values (prices).
        hold_signals (list): Signals for holding actions.
        buy_signals (list): Signals for buying actions.
        sell_signals (list): Signals for selling actions.
        current_portfolio_value (float): Current portfolio value.
        initial_portfolio_value (float): Initial portfolio value.
        dates (list): List of dates.
        total_gains (float): Total gains so far.
        total_losses (float): Total losses so far.
        place_buy_signals (list): Signals for placing pending buy orders.
        place_sell_signals (list): Signals for placing pending sell orders.
        pending_orders_signals (list): Signals for executing pending orders.
        sell_instant_signals (list): Signals for instant selling actions.
        sell_when_feasible_1_signals (list): Signals for selling under condition 1.
        sell_when_feasible_2_signals (list): Signals for selling under condition 2.
        reward (float): Reward for the current action.

    Returns:
        dict: A data point for the current state.
    """
    # Append data points for the plot
    x_vals.append(dates[t])
    y_vals.append(price)

    # Add signals based on the action
    # if action == 0:  # Hold
    #     hold_signals.append((dates[t], price))
    # elif action == 1:  # Buy
    #     buy_signals.append((dates[t], price))
    # elif action == 2:  # Sell (General)
    ...
    # elif action == 6:  # Sell Instantly
    #     sell_instant_signals.append((dates[t], price))
    # elif action == 7:  # Sell When Feasible 1
    #     sell_when_feasible_1_signals.append((dates[t], price))
    # elif action == 8:  # Sell When Feasible 2
    #     sell_when_feasible_2_signals.append((dates[t], price))

    # Determine marker color based on overall profit/loss
    net_profit = current_portfolio_value - initial_portfolio_value
    marker_color = 'green' if net_profit > 0 else 'red'

    # Check if the current date is the start of a new month
    is_month_start = t > 0 and dates[t].month != dates[t - 1].month

    # Construct the data_point dictionary
    data_point = {
        "date": dates[t],
        "price": price,
        "action": action,
        ...
        "signals": {
            ...
            "sell_instant": sell_instant_signals,
            "sell_when_feasible_1": sell_when_feasible_1_signals,
            "sell_when_feasible_2": sell_when_feasible_2_signals,
        },
        "portfolio_info": {
