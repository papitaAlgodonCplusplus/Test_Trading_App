...
def obtain_data():
    ...
    for dp in data_points:
        if dp.get("date") and dp.get("price") is not None:
            ...
            rewards.append(dp.get("reward", 0))
    ...
    sorted_data = sorted(zip(dates, prices, actions, rewards), key=lambda x: x[0])
    dates[:], prices[:], actions[:], rewards[:] = zip(*sorted_data)
...
def update_dashboard_and_table(n_intervals, stored_data):
    ...
    for month_start in updated_data["month_starts"]:
        if month_start not in [line["x0"] for line in vertical_lines]:
            ...
            monthly_portfolio_values.append(portfolio_info["monthly_portfolio_value"])
    ...
    table_fig = go.Figure(
        data=[
            go.Table(
                header=dict(
                    values=["Month", "Monthly Balance (₡)", "Profit (₡)", "Loss (₡)"],
                    fill_color="black",
                ...
                cells=dict(
                    values=[
                        month_names,
                        [round(value, 2) for value in monthly_portfolio_values],
