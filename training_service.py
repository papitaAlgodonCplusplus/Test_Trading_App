...
def train_model(model_name, _, window_size, num_episode, initial_balance, stock_prices, dates):
    ...
    for e in range(1, num_episode + 1):
        ...
        for t in range(1, trading_period + 1):
            ...
            execute_simulaiton(agent, agent_service, actions, stock_prices, dates, trading_period, x_vals, y_vals, hold_signals, buy_signals, sell_signals, action, dates_list, e, returns_across_episodes, num_experience_replay, t, action_dict, next_state, state, reward, t == trading_period)
...
def execute_simulaiton(agent, agent_service, actions, stock_prices, dates, trading_period, x_vals, y_vals, hold_signals, buy_signals, sell_signals, action, dates_list, e, returns_across_episodes, num_experience_replay, t, action_dict, next_state, state, reward, done):
       
    act_and_calculate(agent, agent_service, actions, stock_prices, t, action)
    
    updated_data = update_plot(t, stock_prices[t], action, x_vals, y_vals, hold_signals, buy_signals, sell_signals, agent_service.current_portfolio_value, agent_service.monthly_portfolio_value, dates, agent_service.total_gains, agent_service.total_losses, agent_service.monthly_gains, agent_service.monthly_losses, agent_service.initial_portfolio_value)
    ...
    if done:
        portfolio_return = RewardService.calculate_reward(agent, agent_service.current_portfolio_value, agent_service.monthly_portfolio_value)
        returns_across_episodes.append(portfolio_return)

def act_and_calculate(agent, agent_service, actions, stock_prices, t, action):
    execution_result = agent_service.execute_action(action, actions, stock_prices, t)
